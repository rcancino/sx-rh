...
<content tag="subHeader">
	...
	<g:link action='edit' id="${grupo?.id}">
			...
			<div class="col-lg-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-title">
						<lx:backButton label="Grupo: ${grupo.id}" action="edit" id="${grupo.id}"/>
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#asignar").on('click',function(){
				...
				var ok=confirm('Agregar  ' + res.length+' empleado(s) al grupo:'+${grupo.id}+'?');
				if(!ok)
					return;
				$.post(
					"${createLink(action:'agregarPartidas')}",
					{id:${grupo.id},empleadosIds:JSON.stringify(res)})
				.done(function(data){
					console.log('Rres: ' + data.documento);
					window.location.href='${createLink(action:'edit',params:[id:grupo.id])}';
