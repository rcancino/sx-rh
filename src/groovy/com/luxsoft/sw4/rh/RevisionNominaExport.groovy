...
class RevisionNominaExport {
	...
	String query="""
		SELECT E.id,E.clave,CONCAT(ifnull(E.apellido_paterno,'')," ",ifnull(E.apellido_materno,'')," ",E.nombres) AS nombre,P.numero_de_trabajador,e.alta,(SELECT b.fecha FROM baja_de_empleado b where b.empleado_id=e.id and e.alta<b.fecha ) as baja 
			...
			,max(case when c.clave='P037' THEN NED.importe_gravado ELSE 0 END)  AS BONO_PUNTUALIDAD_ASIST
			...
			,max(case when c.clave='P038' THEN NED.importe_gravado ELSE 0 END)  AS DIA_DESCANSO
		...
		temp.with{
			...
			if(!empresa.rfc.equals("PAP830101CR3")){
				...
				sql.eachRow(query,[id]){ row->
						...
						def percepcionE=0.0
						def percepcionG=0.0
						def totalPercepcion=0.0
						def deduccion=0.0
						def total=0.0
						ne.conceptos.each{ con ->

							if(con.concepto.tipo=='PERCEPCION'){
								percepcionE=percepcionE+con.importeExcento
								percepcionG=percepcionG+con.importeGravado
								totalPercepcion=percepcionE+percepcionG
							}

							if(con.concepto.tipo=='DEDUCCION'){
								deduccion=deduccion+con.importeExcento
							}
							total=totalPercepcion-deduccion
						}

				append(row.id+","+row.clave+","+row.nombre+","+row.numero_de_trabajador+","+row.alta+","+row.baja+","+row.ubicacion+","+row.tipo+","+row.forma_de_pago+","+row.periodicidad+","+row.folio+","+row.periodo_fecha_inicial+","+row.periodo_fecha_final+","+row.dias_del_periodo+","+row.idNe+","+row.salario_diario_base+","+row.salario_diario_integrado+","+row.dias_trabajados+","+row.dias_vac+","+row.incapacidades+","+row.faltas+","+row.fraccion_descanso+","+row.SUELDO+","+row.COMISIONES+","+row.INCENTIVO+","+row.VACACIONES+","+row.VACACIONES_P+","+row.PRIMA_VAC_E+","+row. PRIMA_VAC_G+","+row.PRIMA_ANT_E+","+row.PRIMA_ANT_G+","+row.PRIMA_DOM_E+","+row. PRIMA_DOM_G+","+row.AGUINALDO_E+","+row.AGUINALDO_G+","+row.PTU_E+","+row.PTU_G+","+row.INDEMNIZACION_E+","+row.INDEMNIZACION_G+","+row.HRS_EXTRAS_DOBLES_E+","+row.HRS_EXTRAS_DOBLES_G+","+row.HRS_EXTRAS_TRIPLES+","+row.COMPENSACION+","+row.BONO+","+row.BONO_ANT+","+row.BONO_PUNTUALIDAD_ASIST+","+row.BONO_PRODUCT+","+row.BONO__DESEMP+","+row.GRATIFICACION+","+row.PERMISO_PATERNID+","+row.OTRAS_E+","+row.OTRAS_G+","+row.DIA_DESCANSO+","+row.SUB__EMP_PAG_E+","+row.OTRAS_DEV_ISPT_E+","+row.COMPENSACION_SAF_E+","+row.SUB_EMP_APLIC+","+row.IMSS+","+row.ISR+","+row.RETARDOS+","+row.PENSION_ALIMENT+","+row.ANTICIPO+","+row.INFONAVIT+","+row.INFONACOT+","+row.PRESTAMO+","+row.OTRAS+","+row.RET_COMPENSACION_SAF+","+percepcionE+","+percepcionG+","+totalPercepcion+","+deduccion+","+total+"\r\n" )
			}
